...
export default function SkillsSection() {
  ...
  const allSkills = [
    ...
    { name: "Next.js", icon: SiNextdotjs, color: "oklch(0.95 0 0)", category: "frontend" as const },
    ...
    { name: "Express.js", icon: SiExpress, color: "oklch(0.65 0 0)", category: "backend" as const },
  ...
  return (
    <section id="skills" className="py-24 px-4 relative overflow-hidden">
      ...
      <div className="container mx-auto max-w-5xl">
        ...
        <div className="flex justify-center gap-2 mb-12 flex-wrap animate-on-scroll">
          {tabs.map((tab) => (
            <button
              key={tab.key}
        ...
        <div className="flex flex-wrap justify-center gap-4 md:gap-6">
          {filtered.map((skill, index) => {
            ...
            return (
              <div
                key={index}
                className="group relative flex flex-col items-center justify-center size-24 md:size-28 rounded-2xl bg-card border border-border/50 transition-all duration-300 hover:scale-110 animate-on-scroll"
              ...
              >
                ...
                <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 px-3 py-1.5 rounded-lg bg-card border border-border/50 text-xs text-muted-foreground whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all duration-200 pointer-events-none z-10 shadow-xl backdrop-blur-sm">
                  {skill.name}
                </div>
              </div>
            )
